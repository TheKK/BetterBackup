...
runParallelRestore
  :: (E.IOE E.:> es, Repository E.:> es, StructuredConcurrency E.:> es)
  => (S.Config -> S.Config)
  -- ^ Control how 'parMapM' behaves
  -> E.Eff (RepositoryRestore : es) a
  -> E.Eff es a
runParallelRestore par_map_config emitter = do
  -- TODO Can't tell how large this should be so just pick one randomly.
  tbq <- liftIO $ newTBQueueIO 20

  EKi.scoped $ \scope -> do
    emitter_thread <- EKi.fork scope $ E.evalStaticRep (RepositoryRestoreRep tbq) emitter
    reciver_thread <- EKi.fork scope $ reciver tbq (void $ EKi.await emitter_thread)
    emitter_ret <- liftIO $ atomically $ EKi.await emitter_thread
    () <- liftIO $ atomically $ EKi.await reciver_thread
    pure emitter_ret
  where
...
restoreTreeInBFS abs_root_path root_digest = do
  ...
  EKi.scoped $ \scope -> do
    tree_digest_tq <- liftIO newTQueueIO
    liftIO $ atomically $ writeTQueue tree_digest_tq (abs_root_path, root_digest)

    num_of_working_thread_tvar <- liftIO $ newTVarIO (0 :: Word32)
    let
      take_job = modifyTVar' num_of_working_thread_tvar succ
      done_job = modifyTVar' num_of_working_thread_tvar pred
      running_bracket = bracket_ (pure ()) (liftIO $ atomically done_job)
      no_one_is_running = check . (0 ==) =<< readTVar num_of_working_thread_tvar

    replicateM_ 4 {- CORE/2 -} $ EKi.fork scope $ fix $ \rec -> do
      opt_digest <- liftIO $ atomically $ do
      ...
      case opt_digest of
        Just (parent_abs_path, parent_digest) -> do
          ...
          rec
        Nothing -> pure ()
    liftIO $ atomically $ EKi.awaitAll scope
