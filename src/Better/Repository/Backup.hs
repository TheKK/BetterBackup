...
import qualified Better.Test as Chunker
...
data Ctr = Ctr
  { ctr_aes :: {-# UNPACK #-} !Cipher.AES128
  , ctr_iv :: {-# UNPACK #-} !(TVar (Cipher.IV Cipher.AES128))
  }
...
mk_sem_for_dir = fmap (fromMaybe 4 . readMaybe) (getEnv "CORE") >>= atomically . newTSem

{-# NOINLINE mk_sem_for_file #-}
mk_sem_for_file :: IO TSem
mk_sem_for_file = fmap (fromMaybe 4 . readMaybe) (getEnv "CORE2") >>= atomically . newTSem

fork_or_not :: MonadUnliftIO m => TSem -> m a -> m (m a)
fork_or_not s = \io -> do
  to_fork <- atomically $ (True <$ waitTSem s) <|> pure False
  if to_fork
    then do
      a <- async $ io `finally` atomically (signalTSem s)
      pure $ wait a
    else do
      r <- io
      pure $ pure r

data ForkFns m = ForkFns (m (Digest SHA256) -> m (m (Digest SHA256))) (m (Digest SHA256) -> m (m (Digest SHA256)))
...
backup_dir ctr fork_fns@(ForkFns file_fork_or_not dir_fork_or_not) tbq rel_tree_name = withEmptyTmpFile $ \file_name' -> do
  (dir_hash, ()) <- Un.withBinaryFile (Path.fromAbsFile file_name') WriteMode $ \fd -> do
    Dir.readEither rel_tree_name
      & S.mapM
        ( \fod -> do
            task <- either (file_fork_or_not . backup_file ctr fork_fns tbq . (rel_tree_name </>)) (dir_fork_or_not . backup_dir ctr fork_fns tbq . (rel_tree_name </>)) fod
            pure $ fmap (tree_content fod) task
        )
...
backup_chunk (Ctr aes tvar_iv) tbq chunk = do
  let !chunk_length = runIdentity (S.fromList chunk & fmap Array.length & S.fold F.sum)
  ...
  encrypted_chunk <-
    let !_ = iv == iv
    in encryptCtr aes iv (1024 * 32) (S.fromList $ fmap ArrayBA chunk)
      & fmap un_array_ba
      & S.toList
...
backup dir = do
  ...
  fork_fns <- do
    ...
    pure $ ForkFns (fork_or_not sem_for_file) (fork_or_not sem_for_dir)
