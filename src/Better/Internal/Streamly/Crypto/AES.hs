...
encryptCtr :: (BlockCipher cipher, BA.ByteArray ba) => cipher -> Cipher.IV cipher -> Int -> D.Stream IO ba -> D.Stream IO ba
...
unsafeEncryptCtr :: (BlockCipher cipher, BA.ByteArray ba) => cipher -> Cipher.IV cipher -> Int -> D.Stream IO ba -> D.Stream IO ba
unsafeEncryptCtr cipher iv0 batch_size (D.Stream inner_step inner_s0) = D.Stream step s0
  where
    ...
    step ~st (s', iv, Nothing) = do
      ret <- inner_step st s'
      case ret of
        D.Yield ba next_s -> pure $ D.Yield (Cipher.ctrCombine cipher iv ba) (next_s, Cipher.ivAdd iv iv_offset, Nothing)
        D.Skip next_s -> pure $ D.Skip (next_s, iv, Nothing)
        D.Stop -> pure D.Stop
    step ~_ (s', iv, Just ivv) = pure $ D.Yield (BA.convert ivv) (s', iv, Nothing)
{-# INLINE unsafeEncryptCtr #-}

decryptCtr :: (BlockCipher cipher, BA.ByteArray ba) => cipher -> Int -> D.Stream IO ba -> D.Stream IO ba
...
-- | Unsafe: size of input @ba@ should always has size of @batch_size@ except that last chunk.
unsafeDecryptCtr :: (BlockCipher cipher, BA.ByteArray ba) => cipher -> Int -> D.Stream IO ba -> D.Stream IO ba
        ...
        D.Yield ba next_s -> do
          ...
          pure $
            if BA.length ba == Cipher.blockSize cipher
              ...
              else D.Yield (Cipher.ctrCombine cipher iv $ BA.drop iv_len ba) (next_s, DecDec $ Cipher.ivAdd iv (iv_offset - 1))
        D.Skip next_s -> pure $ D.Skip (next_s, DecReadIV)
        D.Stop -> pure D.Stop
    step ~st (s', DecDec iv) = do
      ret <- inner_step st s'
      case ret of
        D.Yield ba next_s -> pure $ D.Yield (Cipher.ctrCombine cipher iv ba) (next_s, DecDec $ Cipher.ivAdd iv iv_offset)
...
compact n (D.Stream inner_step inner_s0) = D.Stream step s0
  where
    ...
    step ~st (opt_s, buf, begin) = do
      ...
      flip fix (marr0, BufReadEnv (inner_step st) opt_s buf begin) $ \(~rec) (marr, buf_read_env) -> do
